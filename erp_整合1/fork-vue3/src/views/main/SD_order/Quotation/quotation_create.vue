        ...
        <el-row><el-col span="4">
          ...
          <el-col span="20">
            <div style="margin-left: 0px">
              <el-form-item style="margin-left: 0px" prop="quotationId"><el-input v-model="form.quotationId"
                  disabled></el-input>
              </el-form-item>
            </div>
          </el-col></el-row>
  ...
  <div>
    <el-card>
      ...
      <el-table :data="detailData" stripe style="width: 100%" @row-click="getOneMaterial">
        ...
        <el-table-column prop="netValue" label="Net Value" width="120"><template
            #default="scope">{{ scope.row.netValue.toFixed(2) }}</template></el-table-column>
        ...
        <el-table-column label="Description" width="110"><template
            #default="scope"><span>{{ scope.row.name1 }}</span></template></el-table-column>
        <el-table-column label="Net Value" width="100"><template
            #default="scope"><span>{{ scope.row.price1 }}</span></template></el-table-column>
        <el-table-column label="Net weight" width="110"><template
            #default="scope"><span>{{ scope.row.net_weight1 }}</span></template></el-table-column>
          ...
          <el-select v-model="form1.materialId" placeholder="choose material" clearable filterable
            ...
            <el-option v-for="item in tableData" :label="item.materialId" :value="item.materialId">
              ...
              <el-popover placement="right" width="300" trigger="hover">
                ...
                <span>description：{{ item.price }}</span>
                <template #reference>
...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    this.path = to.path;
    if (to.path === "/all_price" || to.path === "/price") { from.meta.keepAlive = true; console.log(to.path); }
    else {
      //from.meta.keepAlive =false;console.log(to.path);
    }
    next();
  },
  data() {
  ...
  methods: {
    ...
    cancelOperation() {
      this.$confirm('Your operation may not have been saved. Do you want to continue?', 'Tip', {
        ...
        if (this.$route.query.method === '3') {
          if (this.justDisplay === true) { this.$router.go(-1) }
          else {
            this.notEdit = true;
            this.justDisplay = true;
          }
    ...
    saveOrUpdate() {
      ...
      if (this.justDisplay === false && this.detailData.length > 0 && this.form.soldToParty && this.form.reqDelivDate) {
        ...
        //debugger
        axios
          .get(this.module + '/quotation/saveQuotation', {
          ...
          .then((response) => {
            ...
            if (this.$route.query.method !== '3') { this.$message({ showClose: true, type: 'success', message: 'Quotation   ' + response.data.data + '   Successfully created!' }) }
            else { this.$message({ showClose: true, type: 'success', message: 'Quotation  ' + response.data.data + '   Successfully modified!' }) }
            this.getQuotation();
          })
          .catch((error) => {
            console.log(error);
    ...
    getQuotation() {
      //这个函数根据上一个页面传输的Quotation_id进行查找
      let needId = parseInt(this.$route.query.quotation_id)
      var that = this;
      axios
        .get(this.module + '/quotation/getQuotationById', {
          params: {
            id: needId
          },
        })
        .then((response) => {
          console.log("获取指定报价单成功");
          console.log(response.data);
    ...
    getDetail() {
      if (this.$route.query.method !== '1') {
        //这个函数根据上一个页面传输的Quotation_id进行detail的查找
        let needId = parseInt(this.$route.query.quotation_id)
        var that = this;
        axios
    ...
    all_discount() {
      let Id = '';
      Id = this.$route.query.quotation_id;
      // 跳转路由
      this.$router.push({
        path: '/all_price',
        query: {
          id: this.form.quotationId,
          type: 2,
          batch: this.mode
        }
      })
    ...
    save(row) {
      let needQuotation = ''
      if (this.$route.query.method === '1') needQuotation = 0
      else needQuotation = this.form.quotationId
      axios
        .get(this.module + '/details/saveDetailsInitial', {
          params: {
    ...
    deleteDetail(row) {
      this.$confirm('Are you sure to delete this material?', 'Tip', {
        ...
        axios
          ...
          .then((response) => {
            console.log(response.data);
            console.log("删除报价单条目成功"); this.detailData = []; this.getDetail();
