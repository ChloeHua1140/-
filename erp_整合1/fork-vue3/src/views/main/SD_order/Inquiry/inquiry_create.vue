<template>
    ...
    <el-row>
      ...
      <el-col :span="12" justify="end">
        <el-steps :space="200" :active="0" simple>
          <el-step title="Inquiry" icon="Document"></el-step>
          <el-step title="Quotation" icon="Edit" />
    ...
    <div>
      <el-card>
        <div>
          ...
          <el-form :model="form" ref="form" class="input_box" label-width="100px" :rules="rules" style="width: 100%"
            :disabled="justDisplay">
            <el-row><el-col span="4">
                <div class="inputDeep">
                  <el-input v-model="type_name" disabled></el-input>
                </div>
              </el-col>
              <el-col span="20">
                <div style="margin-left: 0px">
                  <el-form-item style="margin-left: 0px">
                    <el-input v-model="form.inquiryId" prop="inquiryId" disabled>
            ...
            <el-row>
              ...
              <el-col :span="12">
                <div style="width: 80%">
                  ...
                  <el-form-item label="Customer Reference：" prop="customer_reference" label-width="180px">
                  ...
                  <el-row><el-col :span="18">
                      <el-form-item label="net value：" prop="net_value" label-width="180px">
                        <el-row>
                          ...
                          <el-col :span="4">
                            <div style="background-color: #F5F7FA;color:#A8ABB4;border-radius: 5px"><span>.00</span></div>
                          </el-col>
                        </el-row>
                      </el-form-item></el-col>
                    <el-col :span="4"><el-form-item prop="currency" label-width="20px"><el-input
                          v-model="form.currency"></el-input></el-form-item></el-col></el-row>
                  <el-form-item label="Req.Deliv.Date：" prop="reqDelivDate" label-width="180px">
                    <el-date-picker v-model="form.reqDelivDate" type="date" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
                      placeholder="Choose request delivery date" style="width: 100%" />
                  </el-form-item>
                  <el-row><el-col :span="18">
                      <el-form-item label="Expect Order Value" prop="expectOrdVal" label-width="180px">
                        <el-row>
                          <el-col :span="20"><el-input v-model="form.expectOrdVal" disabled></el-input></el-col>
                          <el-col :span="4">
                            <div style="background-color: #F5F7FA;color:#A8ABB4;border-radius: 5px"><span>.00</span></div>
                          </el-col>
                        </el-row>
            ...
            <el-row style="margin-top: 20px"><el-col :span="12">
                <el-form-item label="Valid From " prop="validFrom" style="width: 60%">
                  <el-date-picker v-model="form.validFrom" type="date" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
                    placeholder="Choose valid from date" />
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="Valid To " prop="validTo" style="width: 70%">
                  <el-date-picker v-model="form.validTo" type="date" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
                    placeholder="Choose valid to date" @change="checkDate" />
                </el-form-item>
              </el-col></el-row>
          </el-form>
        </div>
      ...
      <el-card style="margin-top: 40px">
        <el-row>
          <span>Material</span>
          <el-button icon="Plus" @click="new_detail" style="margin-left:50px" :disabled="justDisplay" color="#193254"
            plain>Add</el-button>
          <el-button icon="Money" @click="all_discount" style="margin-left:50px" :disabled="justDisplay" color="#193254"
            plain>Condition</el-button>
        ...
        <el-table :data="detailData" stripe style="width: 100%" @row-click="getOneMaterial">
          ...
          <el-table-column prop="netValue" label="Net Value" width="120">
            <template #default="scope"><span>{{ (scope.row.netValue).toFixed(2) }}</span></template>
          </el-table-column>
          ...
          <el-table-column label="Description" width="110"><template #default="scope"><span>{{ scope.row.name1
          }}</span></template></el-table-column>
          <el-table-column label="Net Value" width="100"><template #default="scope"><span>{{ scope.row.price1
          }}</span></template></el-table-column>
          <el-table-column label="Net weight" width="110"><template #default="scope"><span>{{ scope.row.net_weight1
          }}</span></template></el-table-column>
          <el-table-column fixed="right">
            <template v-slot="scope">
              ...
              <el-button icon="Check" @click="save(scope.row)" :disabled="justDisplay"
                v-if="editMaterial === true"></el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
  ...
  <el-dialog v-if="editDialog.show" width="800px" :title="editDialog.title" :close-on-click-modal="false"
    v-model="editDialog.show">
    <div v-if="editDialog.show">
      <el-input v-model="searchContent" placeholder="Search for material" class="input-with-select">
        <template #append><el-button icon="Search" @click="searchMaterial"></el-button></template>
      </el-input>
      <el-form style="margin-top: 20px" v-if="editDialog.show" :model="form1" ref="form1" label-width="120px"
        :rules="rule1">
        <el-form-item label="Material_Item" prop="materialId">
          <!--        对接物料模块查询物料-->
          <el-select v-model="form1.materialId" placeholder="choose material" clearable filterable
            @change="selectMaterial">
            <!--            tableData中是数据库中既有的material表格-->
            <el-option v-for="item in tableData" :label="item.materialId" :value="item.materialId">
              <!--弹出框展示仪器基本信息-->
              <el-popover placement="right" width="300" trigger="hover">
                <span>Description：{{ item.name }}</span><br />
                <span>Net weight：{{ item.netWeight }}</span><br />
                <span>Pirce：{{ item.price }}</span>
                <template #reference>
        ...
        <el-form-item label="Quantity" prop="quantity"><el-input-number v-model="form1.quantity"
...
export default {
  ...
  data() {
    return {
      ...
      rule1: {
        quantity: [
          { required: true, message: 'Input quantity', trigger: 'blur' },
        ],
      },
      justDisplay: false,
      sum: '', total: ''
  ...
  methods: {
    ...
    cancelOperation() {
      this.$confirm('Your operation may not have been saved. Do you want to continue?', 'Tip', {
        ...
        if (this.$route.query.method === '3') {
          if (this.justDisplay === true) { this.$router.back() }
          else {
            this.justDisplay = true;
          }
        }
    ...
    saveOrUpdate() {
      ...
      if (this.justDisplay === false && this.detailData.length > 0 && this.form.soldToParty && this.form.reqDelivDate) {
        ...
        //debugger
        axios
          .get(this.module + '/inquiry/saveInquiry', {
            params: { ...this.form, batch: mode, expectOrdVal: this.sum }
          }
          )
          .then((response) => {
            console.log("询价单修改成功");
            console.log(response.data)
            if (this.$route.query.method !== '3') { this.$message({ showClose: true, type: 'success', message: 'Inquiry   ' + response.data.data + '   Successfully created!' }) }
            else { this.$message({ showClose: true, type: 'success', message: 'Inquiry  ' + response.data.data + '   Successfully modified!' }) }
          })
          .catch((error) => {
            console.log(error);
          })
    ...
    getInquiry() {
      //这个函数根据上一个页面传输的Inquiry_id进行查找
      let needId = parseInt(this.$route.query.inquiryId)
      var that = this;
      axios
        .get(this.module + '/inquiry/getInquiryById', {
          params: {
            id: needId
          },
        })
        .then((response) => {
          console.log("获取询价单成功");
          console.log(response.data)
          that.form = response.data.data; this.getDetail();
        })
    },
    getDetail() {
      if (this.$route.query.method === '3') {
        let needId = ''
        //这个函数根据上一个页面传输的Inquiry_id进行detail的查找
        if (this.$route.query.inquiryId) needId = parseInt(this.$route.query.inquiryId); else needId = 404
        var that = this;
        axios
        ...
        let batchId = parseInt(this.mode);
        var that = this;
        axios
    ...
    searchMaterial() {
      ...
      if (this.searchContent) {
        ...
        if (mark === 1) { this.tableData = newListData; }
        else
    ...
    checkDate() {
      if (this.form.validTo !== null && this.form.validFrom !== null && Date.parse(this.form.validTo) < Date.parse(this.form.validFrom)) {
        this.$message({
          type: 'warning',
          message: 'You cannot set a deadline before the expiration date starts.'
        })
      }
    },
    checkPrice(row) {
    ...
    deleteDetail(row) {
      this.$confirm('Are you sure to delete this material?', 'Tip', {
        ...
        axios
          ...
          .then((response) => {
            console.log(response.data);
            console.log("删除报价单成功"); this.detailData = []; this.getDetail();
    ...
    submit() {
      console.log(this.$route.query.method)
      if (!this.form1.quantity) {
        ...
        if (!this.$route.query.method) {
          ...
          let needId = parseInt(this.$route.query.inquiry_id);
    ...
    edit() { console.log(this.editMaterial); this.editMaterial = true; },
    save(row) {
      let needInquiry = ''
      if (this.$route.query.method === '1') needInquiry = 0
      else needInquiry = this.form.quotationId
      axios
        .get(this.module + '/details/saveDetailsInitial', {
          params: {
